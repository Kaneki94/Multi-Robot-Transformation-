    ...
    private void OnCollisionEnter(Collision collision)
    {
        ...
        if (hit)
        {
			...
			if(this.GetComponent<DamageManager>()){
				if (collision.gameObject.tag == "Asteroid" || collision.gameObject.tag == "Bigship" || collision.gameObject.tag == "Base") {
					Debug.Log (collision.gameObject);
					this.GetComponent<DamageManager> ().ApplyDamage (Damage + 10000, collision.gameObject);
				} else {
					Debug.Log (collision.gameObject);
