...
public static class NGUIEditorTools
{
	...
	static public bool WillLosePrefab (GameObject root)
	{
		...
		//mmmmm	PrefabType type = PrefabUtility.GetPrefabType(root);

		//mmmmm	if (type == PrefabType.PrefabInstance)
		//	{
	...
	static public bool MakeTextureReadable (string path, bool force)
	{
		...
		//mmmmm	if (NGUISettings.trueColorAtlas) settings.textureFormat = TextureImporterFormat.AutomaticTruecolor;
	...
	static bool MakeTextureAnAtlas (string path, bool force, bool alphaTransparency)
	{
		...
		if (force ||
			settings.readable ||
		//mmmmm	settings.maxTextureSize < 4096 ||
		...
		//mmmmm	settings.maxTextureSize = 4096;
			...
			{
		//mmmmm		settings.textureFormat = TextureImporterFormat.ARGB32;
