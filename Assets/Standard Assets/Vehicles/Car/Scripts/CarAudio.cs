...
namespace UnityStandardAssets.Vehicles.Car
{
    ...
    public class CarAudio : MonoBehaviour
    {
        ...
        private void Start()
        {
            if (GetComponent<CarUserControl>() && !usercontrol)
        ...
        private void Update()
        {
            ...
            float camDist = (Player.transform.position - transform.position).sqrMagnitude;

            // stop sound if the object is beyond the maximum roll off distance
            if (m_StartedSound /*&& camDist > maxRolloffDistance * maxRolloffDistance*/ && !usercontrol.IsControlEnable)
            {
                StopSound();
            }
