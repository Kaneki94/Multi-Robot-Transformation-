		...
		if (mActiveTTF != fnt)
		{
			if (mActiveTTF != null)
			{
				...
				if (mFontUsage.TryGetValue(mActiveTTF, out usage))
				{
					...
					if (usage == 0)
					{
					//mmmmm	mActiveTTF.textureRebuildCallback = null;
						mFontUsage.Remove(mActiveTTF);
					}
					else mFontUsage[mActiveTTF] = usage;
				}
				//mmmmm else mActiveTTF.textureRebuildCallback = null;
			}
			...
			//mmmmm	if (!mFontUsage.TryGetValue(mActiveTTF, out usage))
			//mmmmm		mActiveTTF.textureRebuildCallback = OnFontTextureChanged;
#if UNITY_FLASH
