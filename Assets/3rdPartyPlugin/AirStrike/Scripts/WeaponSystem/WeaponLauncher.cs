		...
		if (OnActive) {
			...
			if (Reloading) {
				...
				if (Time.time >= reloadTimeTemp + ReloadTime) {
					...
					if (SoundReloaded) {
						if (audio1) {
							audio1.PlayOneShot (SoundReloaded);
						}
					}
					Ammo = AmmoMax;
				}
				...
				if (Ammo <= 0) {
					...
					if (SoundReloading) {
						if (audio1) {
							audio1.PlayOneShot (SoundReloading);
						}
					}
				}
			}
		...
		if (Ammo > 0) {
			if (Time.time > nextFireTime + FireRate) {
				...
				if (SoundGun.Length > 0) {
					if (audio1) {
                        if(audio1.gameObject.activeInHierarchy) {
                            audio1.PlayOneShot(SoundGun[Random.Range(0, SoundGun.Length)]);
                        }
					}
				}
