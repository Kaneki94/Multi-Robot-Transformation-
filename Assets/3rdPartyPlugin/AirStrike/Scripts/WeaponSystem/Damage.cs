...
public class Damage : DamageBase
{
    ...
    public void Active()
    {
		...
		//mmmmm	if(DestroyAfterObject.GetComponent<ParticleSystem>())
		//mmmmm		DestroyAfterObject.GetComponent<ParticleSystem>().emissionRate = 0;
    ...
    private void NormalDamage(Collision collision)
    {
        ...
        if (collision.gameObject.GetComponent<HR_TrafficCar>())
        {
            collision.gameObject.GetComponent<HR_TrafficCar>().ApplyDamage(Damage);
        }
    }
