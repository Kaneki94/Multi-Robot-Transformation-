...
public class HR_TrafficCar : MonoBehaviour
{
    ...
    public void ApplyDamage(int damage)
    {
        ...
        HP -= damage;
        if (this.gameObject.tag == "TrafficCar")
        {
            Dead();
        }
    ...
    private void Dead()
    {
        if (Effect)
        {
            GameObject obj = (GameObject)GameObject.Instantiate(Effect,new Vector3(transform.position.x,transform.position.y+2,transform.position.z), transform.rotation);
        }
